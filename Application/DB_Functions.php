...
Class Connections {
    ...
    public function __Construct()
    {
	 ...
	 function Upload_lib($word,$type){
$sql = $this->db->prepare("INSERT INTO  words(`words`, `sentiments`)
VALUES(:word,:type)");
...
return 0;
	...
	{
		$sql = $this->db->prepare("select * from admin where name=:name and password=:password");
			/* execute statement */
...
return 0;
    ...
    function Register_Subadmin($f_name,$l_name,$mobile_no,$Email,$username,$password)
    {
          //$status = 0;
	
//$db = new PDO('mysql:host=localhost;dbname=hostelakola;charset=utf8', 'root', '');
$sql = $this->db->prepare("INSERT INTO sub_admin(`first_name`, `last_name`, `mobile`, `email`, `username`, `password`)
VALUES(:f_name,:l_name,:mobile_no,:Email,:username,:password)");
    ...
    function recommend_trast($tour,$user)
	{
        
$sql = $this->db->prepare("INSERT INTO trast(`travel_id`, `user_id`)
VALUES(:tour,:user)");

        $sql->bindParam(':tour', $tour);
